package br.com.gekkou.gekkouapi.authentication.security.config;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain configure(HttpSecurity http) throws Exception {
        ...
        customAuthenticationFilter.setFilterProcessesUrl("/api/auth/session");
        http
                .csrf().disable()
                .sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS)
                .and()
                .authorizeRequests().antMatchers("api/auth/session/**", "/api/token/refresh/**").permitAll()
                .and()
                .authorizeRequests().antMatchers(HttpMethod.POST, "/api/account", "/api/account/create").permitAll()
                .and()
